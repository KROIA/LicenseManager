...
#pragma once
#include "LicenseManager_global.h"
...
// ---------------------------------------------------------------------------
#ifdef NDEBUG
	#define LM_CONSOLE(msg)
	#define LM_CONSOLE_FUNCTION(msg)
#else
	#include <iostream>

	#define LM_DEBUG
	#define LM_CONSOLE_STREAM std::cout

	// Print msg to stdout.
	#define LM_CONSOLE(msg) LM_CONSOLE_STREAM << msg;
	// Print the current function signature followed by msg to stdout.
	#define LM_CONSOLE_FUNCTION(msg) LM_CONSOLE_STREAM << __PRETTY_FUNCTION__ << " " << msg;
#endif
...
// ---------------------------------------------------------------------------
	...
	#define LM_PROFILING_BLOCK_C(text, color) EASY_BLOCK(text, color)
	// Non-scoped profiling block — must be closed with PROFILING_END_BLOCK.
	#define LM_PROFILING_NONSCOPED_BLOCK_C(text, color) EASY_NONSCOPED_BLOCK(text, color)
	// End a non-scoped profiling block.
	#define LM_PROFILING_END_BLOCK EASY_END_BLOCK
	// Profile the enclosing function with an explicit color value.
	#define LM_PROFILING_FUNCTION_C(color) EASY_FUNCTION(color)
	// Scoped profiling block using a color stage name (e.g. Cyan500).
	#define LM_PROFILING_BLOCK(text, colorStage) LM_PROFILING_BLOCK_C(text,profiler::colors::  colorStage)
	// Non-scoped profiling block using a color stage name.
	#define LM_PROFILING_NONSCOPED_BLOCK(text, colorStage) LM_PROFILING_NONSCOPED_BLOCK_C(text,profiler::colors::  colorStage)
	// Profile the enclosing function using a color stage name.
	#define LM_PROFILING_FUNCTION(colorStage) LM_PROFILING_FUNCTION_C(profiler::colors:: colorStage)
	// Assign a name to the current thread in the profiler output.
	#define LM_PROFILING_THREAD(name) EASY_THREAD(name)

	// Capture a named scalar value in the profiler timeline.
	#define LM_PROFILING_VALUE(name, value) EASY_VALUE(name, value)
	// Capture a named text string in the profiler timeline.
	#define LM_PROFILING_TEXT(name, value) EASY_TEXT(name, value)

#else
	#define LM_PROFILING_BLOCK_C(text, color)
	#define LM_PROFILING_NONSCOPED_BLOCK_C(text, color)
	#define LM_PROFILING_END_BLOCK
	#define LM_PROFILING_FUNCTION_C(color)
	#define LM_PROFILING_BLOCK(text, colorStage)
	#define LM_PROFILING_NONSCOPED_BLOCK(text, colorStage)
	#define LM_PROFILING_FUNCTION(colorStage)
	#define LM_PROFILING_THREAD(name)

	#define LM_PROFILING_VALUE(name, value)
	#define LM_PROFILING_TEXT(name, value)
#endif
...
// ---------------------------------------------------------------------------
#define LM_COLOR_STAGE_1 50
#define LM_COLOR_STAGE_2 100
#define LM_COLOR_STAGE_3 200
#define LM_COLOR_STAGE_4 300
#define LM_COLOR_STAGE_5 400
#define LM_COLOR_STAGE_6 500
#define LM_COLOR_STAGE_7 600
#define LM_COLOR_STAGE_8 700
#define LM_COLOR_STAGE_9 800
#define LM_COLOR_STAGE_10 900
#define LM_COLOR_STAGE_11 A100
#define LM_COLOR_STAGE_12 A200
#define LM_COLOR_STAGE_13 A400
#define LM_COLOR_STAGE_14 A700
...
// ---------------------------------------------------------------------------
#define LM_GENERAL_PROFILING_COLORBASE Cyan
#define LM_GENERAL_PROFILING_BLOCK_C(text, color) LM_PROFILING_BLOCK_C(text, color)
#define LM_GENERAL_PROFILING_NONSCOPED_BLOCK_C(text, color) LM_PROFILING_NONSCOPED_BLOCK_C(text, color)
#define LM_GENERAL_PROFILING_END_BLOCK LM_PROFILING_END_BLOCK;
#define LM_GENERAL_PROFILING_FUNCTION_C(color) LM_PROFILING_FUNCTION_C(color)
#define LM_GENERAL_PROFILING_BLOCK(text, colorStage) LM_PROFILING_BLOCK(text, CONCAT_SYMBOLS(LM_GENERAL_PROFILING_COLORBASE, colorStage))
#define LM_GENERAL_PROFILING_NONSCOPED_BLOCK(text, colorStage) LM_PROFILING_NONSCOPED_BLOCK(text, CONCAT_SYMBOLS(LM_GENERAL_PROFILING_COLORBASE, colorStage))
#define LM_GENERAL_PROFILING_FUNCTION(colorStage) LM_PROFILING_FUNCTION(CONCAT_SYMBOLS(LM_GENERAL_PROFILING_COLORBASE, colorStage))
#define LM_GENERAL_PROFILING_VALUE(name, value) LM_PROFILING_VALUE(name, value)
#define LM_GENERAL_PROFILING_TEXT(name, value) LM_PROFILING_TEXT(name, value)
